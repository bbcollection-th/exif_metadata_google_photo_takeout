...
def test_write_and_read_gps(tmp_path: Path) -> None:
    ...
    assert abs(float(metadata.get("GPSLatitude", 0)) - 48.8566) < 0.001
    assert abs(float(metadata.get("GPSLongitude", 0)) - 2.3522) < 0.001
    assert metadata.get("GPSLatitudeRef") == "N"
    assert metadata.get("GPSLongitudeRef") == "E"
