...
✅ **119 tests automatisés** - Fiabilité et robustesse garanties  
...
## 🪟 Scripts Windows (Glisser-Déposer)

**Pour les utilisateurs Windows : traitement ultra-simple par glisser-déposer !**

```bash
# Glissez-déposez votre dossier "Google Photos" directement sur :
google_photos_processor.bat      # Script Batch classique
google_photos_processor.ps1      # Script PowerShell (recommandé)
```

**Avantages :**
- 🖱️ **Glisser-déposer** → Aucune ligne de commande nécessaire
- 🎯 **Menu interactif** → Choisissez vos options facilement  
- ✅ **Vérifications automatiques** → Python, ExifTool, fichiers JSON
- 🔧 **Gestion d'erreurs** → Diagnostic et aide intégrés

> 📖 **Voir [SCRIPTS_README.md](SCRIPTS_README.md)** pour le guide complet des scripts Windows

## 🔧 Workflow de Configuration Automatisée

### Génération automatique d'une configuration sur mesure

```bash
# 1. 🔍 Découvrir automatiquement les champs dans vos données
python tools/discover_fields.py "data/Google Photos/"
# → Analyse vos fichiers JSON Google Photos
# → Génère exif_mapping_autogenerated.json avec tous les champs trouvés

# 2. 🧹 Nettoyer et activer la configuration  
python tools/clean_config_file.py
# → Lit exif_mapping_autogenerated.json
# → Supprime les infos de debug
# → Sauvegarde dans exif_mapping.json (prêt à l'emploi)

# 3. ✅ Exécuter le traitement avec votre config personnalisée
python -m google_takeout_metadata "data/Google Photos/"
# → Utilise automatiquement exif_mapping.json
```

**Avantages :**
- 🎯 **Config sur mesure** → Basée sur vos vraies données Google Photos
- 🚀 **Workflow fluide** → 3 commandes pour une config optimale  
- ✅ **Prêt à l'emploi** → Génère directement le bon nom de fichier
- 🔧 **Personnalisable** → Éditez exif_mapping.json selon vos besoins
...
```bash
# Découverte automatique → Configuration prête à l'emploi
python tools/discover_fields.py "data/Google Photos/"  # Génère _autogenerated.json
python tools/clean_config_file.py                      # Nettoie → exif_mapping.json

# Découverte avec options avancées
...
python tools/clean_config_file.py config/custom.json  # Nettoie un fichier spécifique
```
...
├── 📁 config/
│   ├── exif_mapping.json                    ← Configuration active (12 champs optimisés)
│   ├── exif_mapping_autogenerated.json     ← Config auto-générée (découverte)
│   └── exif_mapping_old.json               ← Backup ancienne config
...
│   └── file_organizer.py                   ← Organisation archived/trashed
├── 📁 tests/
...
--geocode             # Géocodage inverse (API Google Maps)
--verbose             # Logs détaillés (niveau DEBUG)
```
...
### Géocodage avec API Google Maps
```bash
# 1. Configurer la clé API (obligatoire pour --geocode)
export GOOGLE_MAPS_API_KEY="votre-cle-api-google-maps"

# 2. Activer le géocodage 
python -m google_takeout_metadata \
    "data/Google Photos/" \
    --geocode

# Note: Si GOOGLE_MAPS_API_KEY est absente, un warning est affiché
# mais le traitement continue sans géocodage
```
...
3. Créer une clé API
4. Définir `GOOGLE_MAPS_API_KEY=votre-clé` dans votre environnement
...
# Mode très détaillé pour diagnostic
python -m google_takeout_metadata \
    "data/Google Photos/" \
    --verbose \
    --batch

# Test à sec (aperçu sans modification)  
python -m google_takeout_metadata \
    "data/Google Photos/" \
    --dry-run \
    --verbose
...
- `INFO` (défaut) : Progression et résultats
- `DEBUG` (--verbose) : Détails techniques et commandes ExifTool
- `WARNING` : Alertes et problèmes non-bloquants
