...
Usage:
    python clean_config_file.py exif_mapping_config_autogenerated.json
"""
...
def clean_config_file(input_file: Path = Path("config/exif_mapping_autogenerated.json"), output_file: Path = Path("config/exif_mapping_config.json")) -> bool:
    """Nettoie un fichier de configuration en supprimant les infos de debug"""
    if not input_file.exists():
        print(f"❌ Fichier non trouvé : {input_file}")
        return False
    if input_file.suffix.lower() != '.json':
        print(f"❌ Le fichier doit être au format JSON : {input_file}")
        return False
    ...
    clean_config = {
        ...
        "timezone_correction": config.get("timezone_correction", {})
    }
    ...
    for name, mapping in config.get("exif_mapping", {}).items():
        ...
        essential_props = [
            "source_fields", "target_tags_image", "target_tags_video", "default_strategy",
        ...
        if clean_mapping and "source_fields" in clean_mapping and "target_tags_image" in clean_mapping:
            clean_config["exif_mapping"][name] = clean_mapping
            cleaned_count += 1
        if clean_mapping and "source_fields" in clean_mapping and "target_tags_video" in clean_mapping:
            clean_config["exif_mapping"][name] = clean_mapping
            cleaned_count += 1
