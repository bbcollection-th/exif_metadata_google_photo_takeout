...
def _build_people_args(meta: SidecarData) -> List[str]:
...
def _build_albums_args(meta: SidecarData) -> List[str]:
...
def build_exiftool_args(meta: SidecarData, media_path: Path | None = None, use_localtime: bool = False, append_only: bool = True) -> List[str]:
    ...
    # Personnes
    args.extend(_build_people_args(meta))
    
    # Albums
    args.extend(_build_albums_args(meta))
...
def write_metadata(media_path: Path, meta: SidecarData, use_localtime: bool = False, append_only: bool = True) -> None:
    if append_only:
        ...
        if args:
            _run_exiftool_command(media_path, args, _append_only=False, is_video=is_video)
